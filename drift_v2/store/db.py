...
class PredictionEventStore:
    ...
    def load_events_by_source_mode(
        self,
        source_mode: str,
        limit: int = 100_000,
    ...
    def load_unlabeled_events_for_expert(
        ...
        predicates = ["expert_label IS NULL"]
        params: list[Any] = []
        if since is not None:
            predicates.append("created_at >= ?")
            params.append(since)
        if source_mode is not None:
            predicates.append("source_mode = ?")
    ...
    def load_ordered_unlabeled_events_for_expert(
        self,
        limit: int,
        since: str | None = None,
        source_mode: str | None = None,
    ...
    def count_events_by_source_mode(self, source_mode: str) -> int:
        with self._connect() as connection:
    ...
    def event_exists(self, event_id: str) -> bool:
        with self._connect() as connection:
            row = connection.execute(
                "SELECT 1 FROM prediction_events WHERE event_id = ? LIMIT 1",
                (event_id,),
            ).fetchone()
