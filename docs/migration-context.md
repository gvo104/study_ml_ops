...
> **Архив.** Миграция из `pipeline/` в `src/` завершена. Актуальная структура и план развития: [`refactoring-plan.md`](refactoring-plan.md).

Документ фиксирует состояние репозитория перед переносом логики из отдельной структуры `pipeline/` в шаблон проекта `study_ml_ops/`.
